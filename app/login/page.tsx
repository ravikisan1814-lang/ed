...
export default function LoginPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!profile) {
        setNotice({ kind: "error", text: "Login failed — no profile found." });
        return;
      }

      // Approved admin (owner) → go to admin dashboard
      if (profile.status === "approved" && profile.access_level === 1) {
        router.push("/admin");
        router.refresh();
        return;
      }

      // Approved regular user → go to learn page
      ...
      // Pending — show notice, stay on page
      setNotice({
        kind: "info",
  ...
  return (
    <div className="auth-page">
      ...
      <div className="auth-hero">
        ...
        <p>
          Sign in to access your content. Login access is enabled
      ...
      <div className="auth-card" data-testid="auth-card">
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          <button type="submit" className="btn btn-primary auth-submit" disabled={busy}>
            {busy ? "Please wait…" : mode === "signin" ? "Sign in" : "Create account (pending approval)"}
          </button>
        </form>

        {notice && (
          <p className={`auth-notice ${notice.kind === "error" ? "auth-notice-error" : ""}`} role={notice.kind === "error" ? "alert" : "status"}>
            {notice.text}
          </p>
        )}
