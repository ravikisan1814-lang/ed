...
| Task | Status |
| --- | --- |
| Extended `scripts/migrate-content.mjs` to scan all 4 source dirs | Done |
| Added `CLASS_TO_EXAM_GROUP` and `IMPORT_SUBJECT_TO_EXAM_GROUP` mappings | Done |
| Added `richFormatToHtml` for import-data/content rich format | Done |
| Added subject + chapter creation (upsert) for missing hierarchy | Done |
| Added meta-file skipping (`syllabus-topics.json`, `topic-map.json`, etc.) | Done |
| Wired 400 content items across 13 subjects, 41 chapters, 56 sub-chapters | Done |
| Added `scripts/migrate-contents-by-subject.mjs` — subject-grouped variant (excludes migrated-content) | Done |
| Added `scripts/verify-migration.mjs` — cross-exam-group count check | Done |
| Updated `.env.example` with new env var docs | Done |
| Verification: `node --check`, `npx tsc --noEmit`, `npm run lint`, `npm test` all green | Done |

### Run commands
- `npm run migrate:content` — full migration (all 4 source dirs)
