...
3. Optionally set `MIGRATE_SOURCE_DIRS` (comma-separated) to override the source directories.
   Optionally set `MIGRATE_TARGET_EXAM_GROUP` to force a single target (default: auto-detect per source).
   Optionally set `MIGRATE_SOURCE_CLASSES` (comma-separated, default: class-11,class-11e,class-12).
...
## Content migration — all sources WIRING (2026-08-16)

### What was done
- Extended `scripts/migrate-content.mjs` to scan **4 source directories**:
  - `../ravikishan/migrated-content` (136 files — class-11/11e/12)
  - `../ravikishan/backend/content` (134 files — class-11/11e)
  - `../ravikishan/backend/prisma/import-data/content` (191 files — subject/unit/file)
  - `../ravikishan/backend-class12-test/content` (1 file — class-12)

- **Per-file exam group auto-detection**: class-based dirs map class→exam group
  (`class-11`/`class-11e`/`class-12`); `import-data/content` maps subject→exam group.
  No single global `TARGET_EXAM_GROUP` — each file lands in the right group.

- **Dual JSON-format support**: standard `title+notes` (array of strings) and rich
  `import-data/content` format (`title, notes, examples, practice, formulas, keyPoints, summary`)
  via `richFormatToHtml()`.

- **Auto-creates missing subjects + chapters + sub-chapters** via upsert when they don't
  exist in the target exam group (idempotent).

- **Skips meta files**: `syllabus-topics.json`, `topic-map.json`, `syllabus.json`, etc.

- Updated docblock, added `CLASS_TO_EXAM_GROUP`, `IMPORT_SUBJECT_TO_EXAM_GROUP`,
  `chapterNameFromSlug`, `typeNameFromSlug`, `richFormatToHtml` helpers.

### Results (live run — idempotent)
| Metric | Count |
| --- | --- |
| Subjects | 13 |
| Chapters | 41 |
...
| Topics | 400 |
| Content items | 400 |
| Skipped | 63 (meta files + general-knowledge/loksewa path edge cases) |
| Errors | 0 |

Exam groups populated: `class-11`, `class-11e`, `class-12`, `academic-core`, `loksewa-gk`.

### To re-run
1. `.env.local` must have `NEXT_PUBLIC_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`.
2. `npm run migrate:content` (or `node scripts/migrate-content.mjs`) — processes all 4 dirs.
3. `npm run migrate:contents` — subject-based variant: same imports but logs grouped by
   subject first (excludes `migrated-content`; useful for clean per-subject verification).
4. `npm run verify:migration` — cross-exam-group count check.
5. Both scripts are idempotent — safe to re-run.
2. `npm run migrate:content` (or `node scripts/migrate-content.mjs`).
3. Idempotent — safe to re-run.

### Verification steps
1. `node --check scripts/migrate-content.mjs` — no syntax errors.
2. `npx tsc --noEmit` — passes.
3. `npm run lint` — passes (0 errors).
4. `npm test` — 23/23.
5. Verify content items in Supabase: `SELECT count(*) FROM content_items` should be ~400+.
