 ...
 * Requires: access_level = 1 AND status = 'approved' (mirrors RLS).
 */
export async function POST(request: Request) {
  ...
  try {
    const { ingestRawNote } = await import("@/lib/ingestion");
    const result = await ingestRawNote(parsed.body as any);
...
function validateBody(
  ...
  if (record.hint !== undefined) {
    if (typeof record.hint !== "string") return { ok: false, error: "hint must be a string" };
    hint = record.hint;
  }
  ...
  if (record.ownerContact !== undefined) {
    if (typeof record.ownerContact !== "string") return { ok: false, error: "ownerContact must be a string" };
    ownerContact = record.ownerContact;
  }

  let publicTeaser: string | undefined;
  if (record.publicTeaser !== undefined) {
    if (typeof record.publicTeaser !== "string") return { ok: false, error: "publicTeaser must be a string" };
    publicTeaser = record.publicTeaser;
  }

  let variantLabel: string | undefined;
  if (record.variantLabel !== undefined) {
    if (typeof record.variantLabel !== "string") return { ok: false, error: "variantLabel must be a string" };
    variantLabel = record.variantLabel;
  }

  let variantInterface: string | undefined;
  if (record.variantInterface !== undefined) {
    if (typeof record.variantInterface !== "string") return { ok: false, error: "variantInterface must be a string" };
    variantInterface = record.variantInterface;
  }

  return {
    ok: true,
    body: { text, hint, title, accessLevel, ownerContact, publicTeaser, variantLabel, variantInterface },
  };
}
